...
Stage 2B 已完成 pilot、6/5 预算校准、温度 0 的固定配置重复观测和版本 2 脱敏报告。由于当前 18 条固定配置观测的回合数与工具调用数完全一致，下一步先离线提取工具调用路径、错误码序列和查询修正模式，比较三种条件究竟改变了哪些决策细节。完成轨迹差异分析后，再决定扩展任务覆盖还是引入受控随机性；在此之前不继续机械增加相同配置的付费重复。
