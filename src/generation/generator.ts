...
Use stable lowercase kebab-case ids. Option names retain leading dashes. cardinality is one|optional|zero-or-more|one-or-more. inferredType is boolean|string|integer|number|json|path|unknown. confidence is high|medium|low. repeatable is yes|no|unknown. constraint kind is implies|conflicts|range|other; represent minimum or maximum bounds as range.
...
For each decision, kind is option|positional, disposition is allow|deny|defer, and risk is low|medium|high. risk is a level only; put the detailed risk explanation in reason.
Create exactly these required input fields, preserving the nested shape objects: [{"name":"filter","description":"...","required":true,"shape":{"type":"string","minLength":1,"maxUtf8Bytes":4096}},{"name":"source","description":"...","required":true,"shape":{"type":"json-source","allowInline":true,"allowFile":true}}].
Bindings have exactly fixedOptions, endOfOptionsCapabilityId, argv, and stdin. fixedOptions is an array of capability-id strings, containing only the compact-output id. argv entries use exactly field, capabilityId, and position: [{"field":"filter","capabilityId":"<filter positional id>","position":0}]. stdin is exactly {"field":"source","encoding":"json"}. Use the standalone -- capability id as endOfOptionsCapabilityId.
